...
public class MenuInventory implements InventoryHolder {
    ...
    public MenuInventory(ItemStack[] contents, String name, int page) {
        ...
        for(ItemStack i : contents) {
            if(i != null) this.inventory.addItem(i);
        }
    ...
    public MenuInventory(Multiblock multi, boolean recipe) {
        this.page = 0;
        this.item = new ItemBuilder(multi.clickedBlock()).setDisplayName(multi.name()).build();
        if (!recipe) {
            this.inventory = Bukkit.createInventory(this, 45, multi.name());
            for(int i = 0; i < this.inventory.getSize(); i++) {
                this.inventory.setItem(i, new ItemStack(Material.BLACK_STAINED_GLASS_PANE));
            }
            for (int x = 0; x < 3; x++) {
            ...
            this.inventory.setItem(25, new ItemBuilder(multi.clickedBlock()).setDisplayName(multi.name()).setLore(multi.description()).build());
            this.inventory.setItem(20, new ItemStack(multi.clickedBlock()));
            this.inventory.setItem(29, new ItemStack(Material.DISPENSER));
        }
        else {
            this.inventory = Bukkit.createInventory(this, 54, multi.name());
        ...
        if (!recipe) {
            ...
            if (Multiblock.comesFromMultiblock(item.getItem())) {
                ...
                if (m != null) {
                    ...
                    this.inventory.setItem(23, new ItemBuilder(m.clickedBlock()).setDisplayName(m.name()).build());
        ...
        if(Multiblock.comesFromMultiblock(item.getItem())) {
            ...
            if (m != null) {
                ...
                if (m.shaped()) {
                    ...
                    m.recipes().forEach((k, v) -> {
                        if (Utils.isSimilar(k, item.getItem())) recipe.set(v);
                    });
                    ...
                    m.recipes().forEach((k, v) -> {
                        for(ItemStack i : v) {
                            if(Utils.isSimilar(i, item.getItem())) thing.set(k);
                        }
                    });
